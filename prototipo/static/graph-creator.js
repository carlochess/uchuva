...
document.onload = (function(d3, saveAs, Blob, undefined){
  ...
  var GraphCreator = function(svg, nodes, edges){
    ...
    d3.select("#save-input").on("click", function(){
     ...
     /* var svg  = document.getElementById('grafo'),
      ...
      svgAsPngUri(document.getElementById("grafo"), {}, function(uri) {
          var envio = {"proyecto": proyecto, "nodes": thisGraph.nodes, "edges": thisGraph.edges, "imagen": uri};
          var blob = envio;
          ...
          $.ajax({
              ...
              success: function(data, textStatus, jqXHR){
                console.log("Exito");
                console.log(data);
                $.notify(data, {placement: {from: "bottom",align: "left"}});
              },
              error: function(jqXHR, textStatus, errorThrown){
                $.notify("Error al intentar guardar", {placement: {from: "bottom",align: "left"}});
              }
          });
    ...
    d3.select("#download-input").on("click", function(){
      ...
      //var blob = /*new Blob([window.JSON.stringify(*/{"nodes": thisGraph.nodes, "edges": saveEdges}/*)], {type: "application/json;charset=utf-8"})*/;
      var envio = {"proyecto": proyecto, "nodes": thisGraph.nodes, "edges": thisGraph.edges};
      var blob = new Blob([window.JSON.stringify(envio)], {type: "application/json;charset=utf-8"});
          ...
          try{
            ...
            if (newEdges){
                  for(var j = 0 ; j< newEdges.length; j++){
                     thisGraph.edges.push({source : thisGraph.nodes[buscar(newEdges[j].source.id)],
    ...
    d3.select("#run-input").on("click", function(){
      ...
      svgAsPngUri(document.getElementById("grafo"), {}, function(uri) {
          ...
          $.ajax({
              ...
              data: envio,
              success: function(data, textStatus, jqXHR){
                $.notify({message: "Ver ejecución "+data, url: "/build?id="+data}, {placement: {from: "bottom",align: "left"}});
                //console.log(data);
              },
              error: function(jqXHR, textStatus, errorThrown){
                $.notify("Error al intentar ejecutar", {placement: {from: "bottom",align: "left"}});
              }
          });
      });
    });
    ...
    function logger(tipo){
      $.ajax({
          ...
          error: function(jqXHR, textStatus, errorThrown){
            $.notify("Error al intentar obtener "+tipo, {placement: {from: "bottom",align: "left"}});
          }
      });
    }
  ...
  GraphCreator.prototype.svgMouseUp = function(){
    ...
    if (state.justScaleTransGraph) {
      ...
      var xycoords = d3.mouse(thisGraph.svgG.node()),
          d = {id: thisGraph.idct++, title: "Nueva Tarea", x: xycoords[0], y: xycoords[1]};
  ...
  GraphCreator.prototype.updateGraph = function(){
    ...
    thisGraph.circles.classed(consts.circleGClass, true)
     .classed('notready', function(d) { return d.estado === "notready"; })
     .classed('ready', function(d) { return d.estado === "ready"; })
     .classed('prerun', function(d) { return d.estado === "prerun"; })
     .classed('submitted', function(d) { return d.estado === "submitted"; })
     .classed('postrun', function(d) { return d.estado === "postrun"; })
     .classed('done', function(d) { return d.estado === "done"; })
     .classed('error', function(d) { return d.estado === "error"; });
  ...
  */
  ...
  edges.forEach(function(e){
    ...
    console.log(buscar(e.source.id));
    console.log(buscar(e.target.id));
  });*/
