    ...
    angular.module('FileManagerApp').service('apiMiddleware', ['$window', 'fileManagerConfig', 'apiHandler', 
        ...
        ApiMiddleware.prototype.getFileList = function(files) {
            return (files || []).map(function(file) {
                return {path: file.model.fullPath(), id:file.model.id};
            });
        };
		...
		ApiMiddleware.prototype.getFilePathWithId = function(item) {
            return {path: item.model.fullPath(), id:item.model.id};
        };

        ApiMiddleware.prototype.list = function(path, idCarpeta, customDeferredHandler) {
            return this.apiHandler.list(fileManagerConfig.listUrl, { path : this.getPath(path), cwd: idCarpeta}, customDeferredHandler);
        };
        ...
        ApiMiddleware.prototype.upload = function(fileList, path, cwd) {
            if (! $window.FormData) {
        ...
        ApiMiddleware.prototype.getContent = function(item) {
            var itemPath = this.getFilePathWithId(item);
        ...
        ApiMiddleware.prototype.createFolder = function(item, idPadre) {
            var path = item.tempModel.fullPath();
            return this.apiHandler.createFolder(fileManagerConfig.createFolderUrl, {path: path, cwd : idPadre});
        };
