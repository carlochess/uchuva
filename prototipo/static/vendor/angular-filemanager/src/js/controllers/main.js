(function(angular, $) {
    ...
    angular.module('FileManagerApp').controller('FileManagerCtrl', [
        ...
        $scope.modal = function(id, hide, returnElement) {
            var element = $('#' + id);
            element.appendTo("body").modal(hide ? 'hide' : 'show');
        ...
        $scope.move = function() {
            ...
            $scope.apiMiddleware.move($scope.temps, $rootScope.getSelectedId() /*$rootScope.selectedModalPath*/).then(function() {
                $scope.fileNavigator.refresh();
        ...
        $scope.createFolder = function() {
            ...
            var name = item.tempModel.name;
			var idPadre = false;
			if($scope.fileNavigator.c)
				idPadre = $scope.fileNavigator.c.model.id;
            if (!name || $scope.fileNavigator.fileNameExists(name)) {
                return $scope.apiMiddleware.apiHandler.error = $translate.instant('error_invalid_filename');
            }
            $scope.apiMiddleware.createFolder(item,idPadre).then(function() {
                $scope.fileNavigator.refresh();
        ...
        $scope.uploadFiles = function() {
            ...
            var cwd = false;
			if($scope.fileNavigator.c)
				cwd = $scope.fileNavigator.c.model.id;
            $scope.apiMiddleware.upload($scope.uploadFileList, $scope.fileNavigator.currentPath, cwd).then(function() {
                $scope.fileNavigator.refresh();
