(function(angular) {
    ...
    angular.module('FileManagerApp').controller('ModalFileManagerCtrl', 
        ...
        $scope.select = function(item) {
			...
			$rootScope.elemento = item.model;
        };

        $scope.selectCurrent = function() {
            $rootScope.selectedModalPath = $scope.fileNavigator.currentPath;
            $scope.modal('selector', true);
			$rootScope.elemento = $scope.fileNavigator.c.model;
        };
		...
		$rootScope.getSelectedId = function() {
            return $rootScope.elemento.id;
        };
